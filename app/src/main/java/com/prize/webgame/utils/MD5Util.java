...
public class MD5Util {
	 ...
	 * @return boolean
	 */
	public static boolean  isDownComplete(Context curContext , String filePath, String packageName) {
		Log.i("MD5Util","isDownComplete");
		PackageManager pm = curContext.getApplicationContext().getPackageManager();
		PackageInfo pathPackageInfo = pm.getPackageArchiveInfo(filePath,
				PackageManager.GET_ACTIVITIES);
		if (pathPackageInfo != null) {
			Log.i("MD5Util","下载包名一致,需要下载包名："+packageName+"---实际下载："+pathPackageInfo.packageName);
				if (!packageName.equals(pathPackageInfo.packageName)) {
					...
					curContext.getApplicationContext().getContentResolver().delete(
